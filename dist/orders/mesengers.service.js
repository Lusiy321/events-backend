        ...
        this.tg_bot.onText(/\/reviews/, async (msg) => {
            try {
                ...
                if (user &&
                    ...
                    await this.tg_bot.sendMessage(chatId, 'Схоже, що у нашій системі відсутні відгуки від вас на пропозиції замовників. Якщо у вас виникнуть будь-які труднощі або питання, не соромтеся звертатися до нас. Дякуємо за розуміння!', optCont);
                }
                if (user &&
                    Array.isArray(user.accepted_orders) &&
                    user.accepted_orders.length !== 0) {
        ...
        this.tg_bot.on('contact', async (msg) => {
            try {
                ...
                if (user && user.tg_chat === null) {
                    ...
                    await this.tg_bot.sendMessage(chatId, `Дякуємо, ${msg.from.first_name}! Тепер вам будуть надходити повідомлення про нові пропозиції в обраній категорії. Щоб вимкнути оповіщення, виберіть "Меню" та натисніть /stop. Якщо у вас є будь-які питання або потребуєте додаткової допомоги, не соромтеся звертатися!`, optURL);
                }
...
Перейдіть на сайт за посиланням: ${process.env.CODE_LINK}`);
                ...
                switch (action) {
                    ...
                    case 'users':
                        ...
                        if (findOrder.accepted_users.length === 0) {
                            await this.tg_bot.sendMessage(chatId, `На жаль, немає жодних відгуків на цю пропозицію.`, optURL);
                        }
                        else {
                            findOrder.accepted_users.map(async (user) => {
                                const findedUser = await this.userModel.findOne({ _id: user });
        ...
        this.tg_bot.onText(/\/stop/, async (msg) => {
            ...
            await this.tg_bot.sendMessage(chatId, `Ви призупинили отримання сповіщень. Для їх відновлення, будь ласка, натисніть кнопку "Відправити номер телефону".`, optCont);
        });
    }
